...
`;
  ...
  /**
   * 🌀 CONVERT LOCATION TO 3D TOPOLOGY DATA
   * 
   * Maps Inner World depth/connections to Blender visualization coordinates
   * Each location renders as two concentric circles representing dual nature
   * 
   * @param {string} locationKey - Location to visualize
   * @returns {Object} - Topology data for Blender API
   */
  getLocationTopology(locationKey) {
    ...
    // Depth influences circle sizing and positioning
    const depthFactor = location.depth;
    const inverseDepth = 5 - location.depth; // Deeper = smaller inner circle
    
    // Color palette by depth (RGB values)
    const depthColors = {
      0: [0.9, 0.9, 0.8], // Herald Chamber: warm parchment
      1: [0.8, 0.8, 0.9], // Sovereignty Vault / Dust Mines: cool steel
      2: [0.6, 0.5, 0.4], // Wallet Roots / Water Reservoir: earthy brown
      3: [0.4, 0.3, 0.5], // Cross-Chain Bridge / Economic Engine: deep purple
      4: [0.3, 0.2, 0.4], // Redstone Circuit / Memory Vault: darker purple
      5: [0.1, 0.1, 0.15] // Core / Abyss: near-black void
    };
    ...
    return {
      ...
      // Circle 1: Outer ring (represents connections)
      circle1: {
        radius: 100 - (depthFactor * 12), // Deeper = smaller
        segments: Math.max(8, 16 - (depthFactor * 2)), // Deeper = fewer segments
        position: [0, 0, depthFactor * -80], // Deeper = lower Z
        rotation: [0.7, 0.3, depthFactor * 0.4],
        color: depthColors[depthFactor]
      },
      
      // Circle 2: Inner ring (represents location essence)
      circle2: {
        radius: 60 - (inverseDepth * 8), // Deeper = larger inner (inverted)
        segments: 8,
        position: [0, 0, depthFactor * 80], // Deeper = higher Z
        rotation: [0.3, 0.7, -(inverseDepth * 0.3)],
        color: depthColors[depthFactor]
      },
      
      // Environment parameters
      environment: {
        fog_density: depthFactor / 6, // Deeper = more fog
        light_intensity: 1 - (depthFactor / 7), // Deeper = darker
        connection_count: location.connects_to.length
      },
      
      // Text overlay for Blender
      text_overlay: {
        title: locationKey.replace(/_/g, ' ').toUpperCase(),
        depth_label: `DEPTH ${depthFactor}`,
        lore_snippet: location.lore.substring(0, 50) + '...'
      }
    };
  }
}
